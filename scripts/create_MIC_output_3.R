...
pure_tiling_files <- list.files("../data/raw_growth_data/upec_tiling_tables_pure", full.names = TRUE)
...
# do same for the concentrated tiling files
# run the code for all pure upec files
tiling_pure_mic <- map_dfr(pure_tiling_files, ~read_delim(.x)[-1,] %>%
  # make table long using pivot_longer
  pivot_longer(cols = 2:ncol(.), names_to = "concentration", values_to = "od600") %>%
  # extract the concentration using gsub:
    mutate(concentration = as.numeric(gsub("^(.*) µM.*", "\\1", concentration))) %>%
...
# write tiling mics to file
write_csv(tiling_pure_mic, "../data/tiling_test/tiling_pure_mics.csv")
